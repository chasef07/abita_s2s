...
from livekit.agents.evals import (
    JudgeGroup, accuracy_judge, conciseness_judge,
    task_completion_judge, tool_use_judge,
)
...
async def evaluate_call(ctx: JobContext) -> None:
    ...
    try:
        ...
        if not any(item.type == "message" and item.role == "user" for item in history.items):
        ...
        async with asyncio.timeout(EVALUATION_SECONDS):
            judges = JudgeGroup(
                ...
                judges=[
                    task_completion_judge(), accuracy_judge(), tool_use_judge(),
                    conciseness_judge(),
                ],
            )
            ...
            if missing:
                ...
                logger.error("Call evaluation missing judges=%s", ",".join(sorted(missing)))
            else:
